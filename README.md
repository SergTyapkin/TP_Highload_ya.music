...
так что два дата-центра в Москве и один в Магадане будет достаточно:
![](Datacenters.png)
...
Этого в 9 раз больше, чем нам нужно.
...
Необходимо обеспечивать отказоустойчивость в случае выхода диска их строя. Так что будет RAID 10 массив из 3-х дисков + 3-х зеркальных по 128 ГБ.
Для отказоустойчивости будет стоять в каждом дата-центре. Будет использоваться Master-Slave репликация.
...
Значит нам понадобится 8 таких стоек + 8 на зеркальную копию.
В каждой диски будут объединены в RAID 10 массив с распределением фрагментов данных по всем дискам для ускорения чтения и зеркальной копией для отказоустойчивости.
Если один из дисков выйдет из строя, у нас есть копии данных как в других дата-центрах, так и, в первую очередь, на зеркальных дисках в этом же дата-центре,
так что восстановить его можно как копированием данных из другого дата-центра, так и из RAID массива.

Также необходима программа, которая будет быстро находить файлы среди этих 700Tb.
С такой задачей призваны справляться S3-хранилища. Здесь у нас, опять же, просто нет выбора,
кроме как использовать [Yandex Object Storage](https://cloud.yandex.ru/services/storage).
Альтернативой мог быть популярный Amazon S3. Наше хранилище отвечает многим требованиям безопасности по шифрованию,
а так же *"реплицировано по географически распределённым зонам доступности"*

Но раз уж мы снимаем свои дата-центры, и я уже рассчитал кол-во характеристик сервера, с которого это все должно раздаваться,
то будем хранить все эти статические файлы в ещё одной `MongoDB`, запущенной на наших серверах, а не в облаке.  

## Список покупок :)
Напомню, в Москве находится два дата-центра, а в Магадане - один.
В каждом из них будет необходимо поставить:

>Балансировщики, микросервисы и базы данных, которыми они пользуются, продублированы для обеспечения отказоустойчивости

Тип сервера                       | Кол-во | CPU (ядра) | RAM (Gb) | SSD (Gb) |
 ---------------------------------| -------| -----------| ---------| ---------| 
Nginx-балансировщик               | 2      | 4          | 16       | 128
Микросервис                       | 6      | 24         | 32       | 24
Redis (сессии)                    | 2      | 8          | 16       | 32
PostgreSQL (пользователи)         | 2      | 8          | 16       | 32
MongoDB (остальные таблицы)       | 2      | 32         | 64       | 768
MongoDB (треки, картинки)         | 2      | 32         | 64       | 1400K
Всего                             | -      | 54         | 104      | 768 + 1400K в RAID 10

*Для разреплицированным по нескольким дата-центрам баз данных будет использоваться Master-Slave репликация.*
